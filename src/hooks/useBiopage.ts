...
export function useBiopage(translations: Record<string, string>) {
    ...
    const saveBiopage = useCallback(async () => {
        ...
        try {
            ...
            toast.error(`${translations.saveError}: ${error}`, { richColors: true, position: 'top-center' });
        }
    }, [biopage, selected, bgColor, textColor, backgroundPositionX, backgroundPositionY, backgroundBlur, backgroundZoom, backgroundImageUrl, avatarUrl, description, translations]);
