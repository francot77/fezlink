...
import type { InsightPeriod, WorkerConfig } from '@/types/insights.types';
...
export async function runInsightsWorker(
  ...
  for (let i = 0; i < pendingCaches.length; i += 5) {
    ...
    const results = await Promise.allSettled(
      batch.map((cache) => processUserInsights(cache.userId, cache.period, workerConfig))
    );
...
async function processUserInsights(
  ...
  try {
    ...
    const needsRecalc =
      !existingCache || !existingCache.inputsHash || existingCache.inputsHash !== inputsHash;
