...
function analyzeGeography(
    ...
    const significant = metrics.topCountries.filter(
        c => c.clicks / total > 0.05
    );
...
function analyzeTemporalPatterns(
    ...
    const peak = metrics.clicksByDayOfWeek.reduce((a, b) =>
...
function analyzeDistributions(
    ...
    if (metrics.topSources.length > 0) {
        ...
        const significant = metrics.topSources.filter(
            s => s.clicks / total > 0.1
        );
    ...
    const hashData = {
        ...
        dailyBins:
            metrics.dailyClicks.length > 0
                ? binTimeSeries(metrics.dailyClicks.map(d => d.clicks), 7)
                : []
    };
