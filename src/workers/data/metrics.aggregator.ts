 ...
 */
function getPeriodDates(period: InsightPeriod): {
  startDate: string;
  endDate: string;
 ...
 */
async function getTopLinks(userId: string, startDate: string, endDate: string, limit: number) {
 ...
 */
async function getTopCountries(userId: string, startDate: string, endDate: string, limit: number) {
  const links = await Link.find({ userId }, { _id: 1 }).lean();
  const linkIds = links.map((l) => l._id);
 ...
 */
async function getTopSources(userId: string, startDate: string, endDate: string, limit: number) {
  const links = await Link.find({ userId }, { _id: 1 }).lean();
  const linkIds = links.map((l) => l._id);
 ...
 */
async function getTopDevices(userId: string, startDate: string, endDate: string, limit: number) {
  const links = await Link.find({ userId }, { _id: 1 }).lean();
  const linkIds = links.map((l) => l._id);
 ...
 */
async function getDailyTimeSeries(userId: string, startDate: string, endDate: string) {
  const links = await Link.find({ userId }, { _id: 1 }).lean();
  const linkIds = links.map((l) => l._id);
 ...
 */
async function getClicksByDayOfWeek(userId: string, startDate: string, endDate: string) {
  const links = await Link.find({ userId }, { _id: 1 }).lean();
  const linkIds = links.map((l) => l._id);
