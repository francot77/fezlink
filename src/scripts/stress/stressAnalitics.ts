...
function randomCountry() {
  return Math.random() < 0.35 ? randomItem(heavy) : randomItem(light);
}
...
async function runWorkers() {
  ...
  while (true) {
    const results = await Promise.all(
      Array.from({ length: PARALLEL_WORKERS }).map(() => runAnalyticsWorker(BATCH_SIZE, CHUNK_SIZE))
    );

    const processed = results.reduce((sum, r) => sum + (r?.processed ?? 0), 0);
  ...
  const avgThroughput = throughputSamples.reduce((a, b) => a + b, 0) / throughputSamples.length;
...
async function verify(linkId: mongoose.Types.ObjectId) {
  ...
  if (remaining !== 0 || link?.totalClicks !== TOTAL_EVENTS || dailyTotal !== TOTAL_EVENTS) {
