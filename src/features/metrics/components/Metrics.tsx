    ...
    return (
        <div className="space-y-4 sm:space-y-6">
                ...
                <div className="relative overflow-hidden rounded-xl sm:rounded-2xl border border-white/10 bg-gradient-to-br from-gray-900/80 via-black/60 to-gray-900/80 backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="relative p-4 sm:p-6">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-white">{t.clickTrend}</h3>
                        </div>
            ...
            {data.deviceTotals.length > 0 && (
                <div className="space-y-3 sm:space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: '100ms', animationFillMode: 'backwards' }}>
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {data.deviceTotals.map((device, index) => {
                            ...
                            return (
                                <div key={device.deviceType} className="animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${index * 50}ms`, animationFillMode: 'backwards' }}>
                                    <MetricCard
            ...
            {data.sourceTotals.length > 0 && (
                <div className="space-y-3 sm:space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: '200ms', animationFillMode: 'backwards' }}>
                    <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-white flex items-center gap-2">
                        🌐 {t.sourceMetrics}
                    </h3>
                    <div className="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {data.sourceTotals.map((entry, index) => {
                            ...
                            return (
                                <div key={entry.source} className="animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${index * 50}ms`, animationFillMode: 'backwards' }}>
                                    <MetricCard
            ...
            {sortedCountries.length > 0 && (
                <div className="space-y-3 sm:space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: '300ms', animationFillMode: 'backwards' }}>
                    <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-white flex items-center gap-2">
                        🌍 {t.countryMetrics}
                    </h3>
                        ...
                        {sortedCountries.map((code, index) => {
                            ...
                            return (
                                <button
                                    ...
                                    title={countryName}
                                >
                                    <div className={`absolute inset-0 bg-gradient-to-br ${filters.country === code
                                    ...
                                    <div className="relative flex items-center justify-between p-3 sm:p-4">
                                        <div className="flex items-center gap-2 sm:gap-3 text-left min-w-0 flex-1">
                                            {/* Flag emoji */}
                                            <div className={`flex h-10 w-10 sm:h-12 sm:w-12 shrink-0 items-center justify-center rounded-lg text-2xl sm:text-3xl transition-all duration-300 ${filters.country === code
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className="text-xs sm:text-sm font-bold text-white truncate" title={countryName}>
                                                    {countryName}
                                                </p>
                                        ...
                                        <div className="text-right shrink-0 ml-2">
                                            ...
                                            <p className="text-xs text-gray-400 hidden sm:block">{t.countryClicks || 'clicks'}</p>
                                        </div>
                                    </div>
